...
def test_softmax_with_ignore_label():
    xpu = mx.cpu()
    ...
    shape = (20, 10)
    x = mx.nd.empty(shape, ctx = mx.cpu())
    l = mx.nd.empty((shape[0],), ctx = mx.cpu())
    x_np = np.random.rand(*shape)
    l_np = np.random.randint(0, shape[1]-1, (shape[0],))
    x[:] = x_np
    l[:] = l_np

    grad = mx.nd.empty(shape, ctx = mx.cpu())
...
if __name__ == '__main__':
    ...
    test_softmax_with_ignore_label(mx.cpu())
